...
**UtmCampaignValue** | **string** | The utm_campaign value applied to the campaign&#39;s tracking links. Falls back to your account&#39;s global UTM settings when no custom value was set on the campaign, or the campaign name if neither is configured. Only returned when UTM tracking is enabled on the campaign. | [optional] [default to null]
